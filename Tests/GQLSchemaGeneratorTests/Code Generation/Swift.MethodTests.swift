...
class SwiftMethodTests: XCTestCase {
    ...
    func testCompleteInit() {
        ...
        XCTAssertEqual(method.name,         Swift.Method.Name.function(.none, "render"))
    ...
    func testMethodNameInit() {
        let init1 = Swift.Method.Name.initialization(.required, true)
        XCTAssertEqual(init1.string, "required init?")
        
        let init2 = Swift.Method.Name.initialization(.convenience, false)
        XCTAssertEqual(init2.string, "convenience init")
        
        let init3 = Swift.Method.Name.initialization(.none, false)
        XCTAssertEqual(init3.string, "init")
        
        let function = Swift.Method.Name.function(.none, "render")
        XCTAssertEqual(function.string, "func render")
    }
    
    func testMethodNameInitializerEquality() {
        let init1 = Swift.Method.Name.initialization(.required, true)
        let init2 = Swift.Method.Name.initialization(.required, true)
        
        XCTAssertEqual(init1, init2)
        
        let init3 = Swift.Method.Name.initialization(.required, false)
        let init4 = Swift.Method.Name.initialization(.none, true)
    ...
    func testMethodNameFunctionEquality() {
        let func1 = Swift.Method.Name.function(.none, "render")
        let func2 = Swift.Method.Name.function(.none, "render")
        
        XCTAssertEqual(func1, func2)
        
        let func3 = Swift.Method.Name.function(.none, "rendering")
